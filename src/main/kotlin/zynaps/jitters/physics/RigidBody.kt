...
import zynaps.jitters.geometry.Shape
...
class RigidBody(val skin: Shape) {
    ...
    val newState = PhysicsState()
    val oldState = PhysicsState()
    ...
    var maxLinearVelocities = Vector3(JConfig.HUGE, JConfig.HUGE, JConfig.HUGE)
        set(value) {
    ...
    var maxAngularVelocities = Vector3(JConfig.HUGE, JConfig.HUGE, JConfig.HUGE)
        set(value) {
            field = Vector3.abs(value)
        }
    ...
    private fun addAngularVelocityToOrientation(angularVelocity: Vector3, orientation: Matrix4, dt: Float): Matrix4 {
        ...
        if (ang <= JConfig.TINY) return orientation
