...
object GjkEpaSolver : CollisionDetector {
    private val gjk = GJK()
    ...
    class Results : CollisionDetails {
        override val pointA: Vector3 get() = witnesses[0]
        override val pointB: Vector3 get() = witnesses[1]
        override var normal = Vector3.ZERO
        override val collided: Boolean get() = status == ResultsStatus.PENETRATING
        override var depth = 0F

        var witnesses = arrayOf(Vector3.ZERO, Vector3.ZERO)
    ...
    private fun transform(t: Vector3, m: Matrix4) = Vector3(
        m.m00 * t.x + m.m01 * t.y + m.m02 * t.z,
        m.m10 * t.x + m.m11 * t.y + m.m12 * t.z,
        m.m20 * t.x + m.m21 * t.y + m.m22 * t.z
    )
        ...
        fun encloseOrigin(): Boolean {
            when (order) {
                ...
                1 -> {
                    ...
                    simplex[2].w = support(Vector3.normalize(transform(w, r)), simplex[2])
                    simplex[3].w = support(Vector3.normalize(transform(w, r)), simplex[3])
                    order = 4
                    return true
                }
                2 -> {
